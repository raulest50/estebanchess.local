...
from chess_move_analyzer.app import (
    ...
    _render_app_shell,
    _render_detail,
    _resolve_mdns_enabled,
    _resolve_mdns_name,
    _resolve_host,
    _resolve_show,
    _host_is_lan_accessible,
    _select_move,
    _select_pv_move,
...
def test_runtime_host_and_show_are_configurable(monkeypatch):
    ...
    monkeypatch.setenv("CHESS_ANALYZER_HOST", "127.0.0.1")
...
def test_mdns_runtime_options_are_configurable(monkeypatch):
    monkeypatch.delenv("CHESS_ANALYZER_MDNS", raising=False)
    monkeypatch.delenv("CHESS_ANALYZER_MDNS_NAME", raising=False)

    assert _resolve_mdns_enabled() is True
    assert _resolve_mdns_name() == "chess109.local"
    assert _host_is_lan_accessible("0.0.0.0") is True
    assert _host_is_lan_accessible("192.168.1.20") is True
    assert _host_is_lan_accessible("127.0.0.1") is False
    assert _host_is_lan_accessible("localhost") is False

    monkeypatch.setenv("CHESS_ANALYZER_MDNS", "false")
    monkeypatch.setenv("CHESS_ANALYZER_MDNS_NAME", "chess-lab.local")

    assert _resolve_mdns_enabled() is False
    assert _resolve_mdns_name() == "chess-lab.local"
